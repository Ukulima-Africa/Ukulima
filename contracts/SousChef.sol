...
contract SousChef {
  ...
  // Info of each user.
  struct UserInfo {
    uint256 amount;   // How many YIELD tokens the user has provided.
    ...
    //
    // Whenever a user deposits or withdraws YIELD tokens to a pool. Here's what happens:
  ...
  // Info of each user that stakes Yield tokens.
  ...
  function pendingReward(address _user) external view returns (uint256) {
    ...
    uint256 stakedSupply = yield.balanceOf(address(this));
  ...
  function updatePool() public {
    ...
    uint256 yieldSupply = yield.balanceOf(address(this));
    if (yieldSupply == 0) {
    ...
    poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(yieldSupply));
    poolInfo.lastRewardBlock = block.number;
  }


  // Deposit Yield tokens to SousChef for Reward allocation.
  function deposit(uint256 _amount) public {
    require (_amount > 0, 'amount 0');
    UserInfo storage user = userInfo[msg.sender];
    updatePool();
    yield.safeTransferFrom(address(msg.sender), address(this), _amount);
  ...
  // Withdraw Yield tokens from SousChef.
  function withdraw(uint256 _amount) public {
    ...
    updatePool();
    yield.safeTransfer(address(msg.sender), _amount);
  ...
  function emergencyWithdraw() public {
    UserInfo storage user = userInfo[msg.sender];
    yield.safeTransfer(address(msg.sender), user.amount);
