...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
  beforeEach(async () => {
    ...
    this.timelock = await Timelock.new(bob, '28800', {
      from: alice
    }); // 8hours
  });

  it('should not allow non-owner to do operation', async () => {
    await this.agri.transferOwnership(this.timelock.address, {
      from: alice
    });
    await expectRevert(
      this.agri.transferOwnership(carol, {
        from: alice
      }),
    ...
    await expectRevert(
      this.timelock.queueTransaction(
        this.agri.address, '0', 'transferOwnership(address)',
  ...
  it('should do the timelock thing', async () => {
    ...
    await this.timelock.queueTransaction(
      this.agri.address, '0', 'transferOwnership(address)',
      encodeParameters(['address'], [carol]), eta, {
        from: bob
      },
    );
    ...
    assert.equal((await this.agri.owner()).valueOf(), carol);
  });
  ...
  it('should also work with MasterChef', async () => {
    ...
    this.chef = await MasterChef.new(this.agri.address, this.yield.address, dev, '1000', '0', {
      from: alice
    });
    ...
    await expectRevert(
      this.chef.add('100', this.lp1.address, true, {
        from: alice
      }),
    ...
    await this.timelock.queueTransaction(
      ...
      encodeParameters(['address'], [minter]), eta, {
        from: bob
      },
    );
    // await this.timelock.queueTransaction(
