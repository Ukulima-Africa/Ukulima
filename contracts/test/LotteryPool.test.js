...
contract('MasterChef', ([alice, bob, carol, dev, minter]) => {
  ...
  it('real case', async () => {
    ...
    await this.lp4.transfer(this.lottery.address, '10', {
      from: minter
    });
    ...
    await this.chef.add('500', this.lp4.address, true, {
      from: minter
    });
    ...
    await this.lottery.startFarming(4, this.lp4.address, '1', {
      from: dev
    });
    ...
    assert.equal(
      (await this.agri.balanceOf(this.lottery.address)).toString(),
      '0'
    );

    await this.lottery.harvest(4, {
      from: dev
    });
    ...
    assert.equal((await this.agri.balanceOf(carol)).toString(), '5');
  });
  ...
  it('setReceiver', async () => {
    ...
    await this.lp1.transfer(this.lottery.address, '10', {
      from: minter
    });
    await this.chef.add('1000', this.lp1.address, true, {
      from: minter
    ...
    await this.lottery.harvest(1, {
      from: dev
    });
    assert.equal((await this.agri.balanceOf(carol)).toString(), '7');
    await this.lottery.setReceiver(alice, {
      from: dev
    });
    ...
    assert.equal((await this.agri.balanceOf(alice)).toString(), '15');
  });
  ...
  it('update admin', async () => {
    ...
    await this.lottery.setAdmin(alice, {
      from: minter
    });
    ...
    await expectRevert(
      this.lottery.startFarming(1, this.lp1.address, '1', {
        from: dev
      }),
      'admin: wut?'
    );
