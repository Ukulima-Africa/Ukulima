...
contract('BnbStaking.......', async ([alice, bob, admin, dev, minter]) => {
  beforeEach(async () => {
    this.rewardToken = await AgriToken.new({
      from: minter
    });
    this.lpToken = await MockBEP20.new('LPToken', 'LP1', '1000000', {
      from: minter,
    });
    this.wBNB = await WBNB.new({
      from: minter
    });
    ...
    await this.rewardToken.mint(this.bnbChef.address, 100000, {
      from: minter
    });
  });
  ...
  it('deposit/withdraw', async () => {
    ...
    await this.bnbChef.withdraw('100', {
      from: alice
    });
    assert.equal(
      (await this.wBNB.balanceOf(this.bnbChef.address)).toString(),
    ...
    await this.bnbChef.emergencyRewardWithdraw(1000, {
      from: minter
    });
  ...
  it('should block man who in blanklist', async () => {
    await this.bnbChef.setBlackList(alice, {
      from: admin
    });
    await expectRevert(
      this.bnbChef.deposit({
        from: alice,
        value: 100
      }),
      'in black list'
    );
    await this.bnbChef.removeBlackList(alice, {
      from: admin
    });
    await this.bnbChef.deposit({
      from: alice,
    ...
    await this.bnbChef.setAdmin(dev, {
      from: minter
    });
    await expectRevert(
      this.bnbChef.setBlackList(alice, {
        from: admin
      }),
      'admin: wut?'
    );
  ...
  it('emergencyWithdraw', async () => {
    ...
    await this.bnbChef.emergencyWithdraw({
      from: alice
    });
    assert.equal(
      (await this.wBNB.balanceOf(this.bnbChef.address)).toString(),
  ...
  it('emergencyRewardWithdraw', async () => {
    await expectRevert(
      this.bnbChef.emergencyRewardWithdraw(100, {
        from: alice
      }),
  ...
  it('setLimitAmount', async () => {
    ...
    await this.bnbChef.setLimitAmount('1000000', {
      from: minter
    });
    await expectRevert(
      this.bnbChef.deposit({
