...
contract('Upgradeable BEP20 token', (accounts) => {
  it('Create Token', async () => {
    ...
    const tx = await BEP20TokenFactoryInstance.createBEP20Token("ABC Token", "ABC", 18, web3.utils.toBN(1e18), true, bep20Owner, proxyAdmin, {
  ...
  it('Test bep20 query methods', async () => {
    ...
    const name = await bep20.methods.name().call({
      from: bep20Owner
    });
    ...
    const decimals = await bep20.methods.decimals().call({
      from: bep20Owner
    });
    assert.equal(decimals, 18, "wrong token decimals");

    const totalSupply = await bep20.methods.totalSupply().call({
      from: bep20Owner
    });
    assert.equal(totalSupply, web3.utils.toBN(1e18), "wrong totalSupply");

    const bep20OwnerBalance = await bep20.methods.balanceOf(bep20Owner).call({
      from: bep20Owner
    });
    assert.equal(bep20OwnerBalance, web3.utils.toBN(1e18), "wrong balance");

    const owner = await bep20.methods.getOwner().call({
      from: bep20Owner
    });
  ...
  it('Test bep20 transaction methods', async () => {
    ...
    const balanceOld = await bep20.methods.balanceOf(accounts[2]).call({
      from: bep20Owner
    });
    assert.equal(balanceOld, web3.utils.toBN(0), "wrong balance");

    await bep20.methods.transfer(accounts[2], web3.utils.toBN(1e17)).send({
      from: bep20Owner
    });

    const balanceNew = await bep20.methods.balanceOf(accounts[2]).call({
      from: bep20Owner
    });
    assert.equal(balanceNew, web3.utils.toBN(1e17), "wrong balance");

    await bep20.methods.approve(accounts[3], web3.utils.toBN(1e17)).send({
      from: bep20Owner
    });

    let allowance = await bep20.methods.allowance(bep20Owner, accounts[3]).call({
      from: accounts[3]
    });
    assert.equal(allowance, web3.utils.toBN(1e17), "wrong allowance");

    await bep20.methods.transferFrom(bep20Owner, accounts[4], web3.utils.toBN(1e17)).send({
      from: accounts[3]
    });

    allowance = await bep20.methods.allowance(bep20Owner, accounts[3]).call({
      from: accounts[3]
    });
    assert.equal(allowance, web3.utils.toBN(0), "wrong allowance");
    const balance = await bep20.methods.balanceOf(accounts[4]).call({
      from: accounts[4]
    });
  ...
  it('Test mint and burn', async () => {
    ...
    let totalSupply = await bep20.methods.totalSupply().call({
      from: bep20Owner
    });
    assert.equal(totalSupply, web3.utils.toBN(1e18), "wrong totalSupply");

    await bep20.methods.mint(web3.utils.toBN(9e18)).send({
      from: bep20Owner
    });

    totalSupply = await bep20.methods.totalSupply().call({
      from: bep20Owner
    });
    assert.equal(totalSupply, web3.utils.toBN(10e18), "wrong totalSupply");

    await bep20.methods.transfer(accounts[5], web3.utils.toBN(2e18)).send({
      from: bep20Owner
    });
    await bep20.methods.burn(web3.utils.toBN(2e18)).send({
      from: accounts[5]
    });
  ...
  it('Test ApproveAndCallFallBack', async () => {
    ...
    await bep20Proxy.methods.upgradeTo(BEP20TokenImplementationV1.address).send({
      from: proxyAdmin
    });
    ...
    await bep20.methods.approveAndCall(ApproveAndCallFallBackTest.address, web3.utils.toBN(1e18), web3.utils.hexToBytes("0x")).send({
      from: bep20Owner
    });
  });
});
